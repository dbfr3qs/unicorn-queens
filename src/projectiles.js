...
export function fireFireball(x, y, vx, vy, fx) {
  fireballs.push({ x, y, w: FIREBALL_SIZE, h: FIREBALL_SIZE, vx, vy, ttl: FIREBALL_TTL, dead: false });
  fx.play('fireball');
}

// Advance fireballs: fizzle on TTL, off-level, or surface contact;
// hurt the player on contact (shared hurt, so invulnerability applies).
export function updateFireballs(p, lvl, cam, dt, fx) {
